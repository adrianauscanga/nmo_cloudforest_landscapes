...
plots_cf_4p %>%
  ...
  ggplot(aes(x = diam_class, y = agb_prop, fill = str_class_plots)) +
  geom_boxplot() +
  theme_classic()
```
...
group_by(diam_class) %>%
  ggdensity("agb_prop", facet.by = "diam_class")
  ggqqplot("agb_prop", facet.by = "diam_class")
...
plots_cf_4p %>%
  ...
  mutate(agb_prop = agb_class_plot/agb_plot)  %>%
  group_by(diam_class) %>%
  kruskal.test(agb_prop ~ diam_class)
...
pairwise.wilcox.test(x= x$agb_prop, g= x$diam_class, p.adjust.method = "bonferroni")

anova <- aov(agb_prop ~ diam_class, data = x)
TukeyHSD(anova)
...
plots_cf_4p %>%
  ...
  right_join(agb_class_plots, by = "plot_id") %>%
  mutate(agb_prop = agb_class_plot/agb_plot)  %>%
  ggplot(aes(x = diam_class, y = agb_plot, fill = str_class_plots)) +
  geom_boxplot() +
  theme_classic()
```
