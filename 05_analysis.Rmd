...
lm <- lm(log(av_agb_site) ~ env_pc1 + slope_gee + aspect_gee_c + shannon_av_site + simpson_av_site + sp_richness_site_av + mosaic_inv + agriculture + grazing, data = sites_cf_4p)
summary(lm)
```
...
```{r}
lm2 <- lm(log(av_agb_site) ~ slope_gee + mosaic_inv + agriculture, data = sites_cf_4p)
summary(lm2)
...
Species richness and AGB in relation to successional stage

Simpson and AGB in relation to successional stage