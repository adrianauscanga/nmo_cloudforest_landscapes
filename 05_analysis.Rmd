...
PC1 explains 68.23% of variation and is correlated to temp, altitude and precip.
...
lm <- lm(log(av_agb_site) ~ env_pc1 + slope_gee + aspect_gee_c + shannon + sp_richness_site + landscape + agriculture + grazing, data = sites_cf_4p)
summary(lm)
```
...
```{r}
ms <- regsubsets(log(av_agb_site) ~ env_pc1 + slope_gee + shannon + sp_richness_site + landscape + agriculture + grazing, data = sites_cf_4p)
...
lm2c <- lm(log(av_agb_site) ~ slope_gee + landscape + agriculture + env_pc1 + grazing + shannon, data = sites_cf_4p)
summary(lm2c)
partial.resid.plot(lm2c)

lm2c_res <- lm2c$residuals #save residuals

summary(lm(landscape ~ shannon, data = sites_cf_4p))
```
...
# 6 predictors
summary(lm(log(av_agb_site) ~ slope_gee + landscape + agriculture + env_pc1 + grazing + shannon, data = sites_cf_4p))

# 7 predictors
summary(lm(log(av_agb_site) ~ slope_gee + landscape + agriculture + env_pc1 + grazing + shannon + sp_richness_site, data = sites_cf_4p))
```
...
loc_subs <- cbind(plots_cf_4p$longitude, plots_cf_4p$latitude)
...
model_res <- gamm4_4_int$gam$residuals

#Moran test
# spatial autocorrelation of model residuals
moran.test(model_res, zero.policy=T, 
...
# Wood density at site level

trees_cf_4p %>%
  filter(!is.na(mean_wd)) %>%
  group_by(site) %>%
  summarise(mean_wd_site = mean(mean_wd),
            se_wd_site = calcSE(mean_wd)) %>%
  arrange(-mean_wd_site) %>%
  mutate(average = mean(mean_wd_site),
         se = calcSE(mean_wd_site))

mean(trees_cf_4p$mean_wd, na.rm = T)
sd(trees_cf_4p$mean_wd, na.rm = T)

# Wood density at plot level
...
trees_cf_4p %>%
  ...
  #group_by(str_class_plots) %>%
  # summarise(mean_wd_str = mean(mean_wd_plot),
  #           sd_wd_str = sd(mean_wd_plot)) %>%
  tukey_hsd(mean_wd_plot ~ str_class_plots)
...
str_k3_plots <- plots_cf_4p$str_class_plots
...
points(pca_str_plots, col = plots_cf_4p$str_class_plots)
...
Proportion of AGB by successional stage

```{r}

plots_cf_4p %>%
  group_by(str_class_plots) %>%
  summarise(agb_total = sum(agb_plot_ha))

# AGB total:
#2303.270+9404.818+10291.113 =
#21999.2

plots_cf_4p %>%
  group_by(str_class_plots) %>%
  summarise(agb_total = sum(agb_plot_ha)) %>%
  mutate(per100 = (agb_total / 21999.2) *100)

```

```{r}
plots_cf_4p %>% filter(str_class_plots == "2") %>% arrange(-agb_plot_ha)
```
