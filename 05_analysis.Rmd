...
```{r}
ms <- regsubsets(log(av_agb_site) ~ env_pc1 + slope_gee + aspect_gee_c + shannon_av_site + simpson_av_site + sp_richness_site_av + landscape + agriculture + grazing, data = sites_cf_4p)
...
sites_cf_4p %>%
  ggplot(aes(x = log(av_agb_site), y = sp_richness_site_av)) +
...
sites_cf_4p %>%
  ggplot(aes(x = log(av_agb_site), y = simpson_av_site)) +
  geom_point(size=2) +
  geom_smooth(alpha = 0.2, method = "loess", span = 1, color= "gray") +
  theme_classic()
...
plots_cf_4p %>%
  ...
  geom_smooth(method = "gam", span = 1, color = "gray", se= T, alpha = 0.1) +
