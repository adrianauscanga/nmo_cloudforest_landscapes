...
disturbance_cf %>%
  ...
  filter(cause == "Uso del suelo diferente al forestal" | #LULCC
           ...
           cause == "Pastoreo" |
           cause == "unknown") %>% # Cattle grazing
  mutate(disturbance_severity = ifelse(on_veg == "Mayor", 4,
                                       ifelse(on_veg == "Mediana", 3,
                                              ifelse(on_veg == "Menor", 2,
                                                     ifelse(on_veg == "No perceptible", 1, 0))))) %>%
  ...
  select(-on_veg, -notes) %>%
  pivot_wider(names_from = cause, values_from = c(disturbance_severity), values_fill = 0) %>%
  mutate(lulcc = ifelse(is.na(unknown), "unknown", lulcc),
         logging = ifelse(is.na(unknown), "unknown", logging),
         fire = ifelse(is.na(unknown), "unknown", fire),
         grazing = ifelse(is.na(unknown), "unknown", grazing)) %>%
  select(-unknown, -state, -municipality) %>%
  right_join(sites_cf, by = "site") %>%
  mutate(lulcc = ifelse(is.na(lulcc), 0, lulcc),
