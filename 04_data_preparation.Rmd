...
plots_cf <- plots_cf %>%
  mutate(northness = cos(aspect),
         eastness = sin(aspect)) %>%
  relocate(c("northness", "eastness"), .after = aspect)
...
aspect_slope_sites <- plots_cf %>%
  select(site, plot_id, aspect, northness, eastness, slope) %>%
  group_by(site) %>%
  summarize(aspect_gee = mean(aspect),
            aspect_se_gee = calcSE(aspect),
            northness_gee = mean(northness),
            northness_se_gee = calcSE(northness),
            eastness_gee = mean(eastness),
            eastness_se_gee = calcSE(eastness),
...
```{r}
sites_cf <- sites_cf %>%
 mutate(landscape_str = 1- mosaic,
        landscape_dist = 1 - mosaic_dist)
```
...
ggdensity(sites_cf2$hill_shannon)

#qqplot
ggqqplot(sites_cf2$av_agb_site)
ggqqplot(sites_cf2$hill_shannon)
```
...
shapiro.test(plots_cf$agb_plot_ha)
shapiro.test(plots_cf$temp)
shapiro.test(plots_cf$precip)
shapiro.test(plots_cf$northness) 
shapiro.test(plots_cf$eastness)
shapiro.test(plots_cf$slope) 
shapiro.test(plots_cf$logging)
shapiro.test(plots_cf$agriculture)
shapiro.test(plots_cf$grazing)
shapiro.test(plots_cf$hill_richness) 
shapiro.test(plots_cf$hill_shannon) 

ggdensity(plots_cf$agb_plot_ha)
ggdensity(plots_cf$temp)
ggdensity(plots_cf$precip)
ggdensity(plots_cf$northness) #
ggdensity(plots_cf$eastness)
ggdensity(plots_cf$slope) 
ggdensity(plots_cf$logging)
ggdensity(plots_cf$agriculture)
ggdensity(plots_cf$grazing)
ggdensity(plots_cf$hill_richness) 
ggdensity(plots_cf$hill_shannon)
...
shapiro.test(sites_cf2$hill_richness) # normal p=0.15
shapiro.test(sites_cf2$hill_shannon) # normal p = 0.60
...
ggdensity(sites_cf2$hill_richness)
ggdensity(sites_cf2$hill_shannon)
...
shapiro.test(log1p(plots_cf$agb_plot_ha))
ggdensity(log1p(plots_cf$agb_plot_ha))
...
```
Replace NAs by 0 when needed

```{r}

# plots_cf <- plots_cf %>%
#   mutate(hill_shannon = ifelse(is.na(hill_shannon), 0, hill_shannon),
#          hill_richness = ifelse(is.na(hill_richness), 0, hill_richness))
# 
