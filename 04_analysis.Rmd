...
sites_cf %>%
  ...
  filter(!is.na(agriculture)) %>%
  filter(!is.na(grazing)) %>%
  ggplot(aes(x = aspect, y = log(av_agb_site))) +
  geom_boxplot()
...
sites_cf %>%
  ...
  filter(!is.na(agriculture)) %>%
  filter(!is.na(grazing)) %>%
  ggplot(aes(x = aspect, y = lm5$residuals)) +
  geom_boxplot()
...
lm15 <- lm(log(av_agb_site) ~ temp + precip + altitude + slope_gee + aspect_gee + simpson_av_site + subregion, data = forest)
summary(lm15)
```
...
forest %>%
  ggplot(aes(x = simpson_av_site, y = av_agb_site)) +
  geom_point() +
  geom_smooth(method = 'lm')
