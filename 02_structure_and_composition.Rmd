...
Calculate aboveground biomass with BIOMASS package and other allometric equations. 
...
#Final tree_cf dataset has 7096 rows out of which 6,231 are trees, the rest are of unknown life form (probably trees judging by their genus) 
...
sites_cf <- plots_cf %>%
  ...
  relocate(c(precip, temp, ai, tree_no_site, tree_no_ave, av_basalarea_site, se_basalarea_site, av_treeheight_site, se_treeheight_site, av_loreys_site, se_loreys_site, av_treedensity_site, se_treedensity_site), .after = latitude)

#Final cleaning

sites_cf <- sites_cf %>%
  select(-c(plots_by_site, plot1, plot2, plot3, plot4, site_type, monitoring, sampled, vegetation_cover, tree_no_site)) %>%
  relocate(c(longitude, latitude, temp, precip, ai), .after = land_tenure) %>%
...
# True diversity
#Transform diversity indices to true diversity
#(for example: https://onlinelibrary.wiley.com/doi/full/10.1111/oik.07202)

shannon <- shannon %>%
  mutate(t_shannon = exp(shannon))
...
diversity_sites <- plots_cf %>%
  mutate(t_shannon = ifelse((is.na(t_shannon)), 0, t_shannon),
         #simpson = ifelse((is.na(simpson)), 0, simpson),
         site = as.character(site)) %>%
  group_by(site) %>%
  summarize(shannon_av_site = mean(shannon),
            shannon_se_site = calcSE(shannon),
            t_shannon_av_site = mean(t_shannon),
            t_shannon_se_site = calcSE(t_shannon))
...
### True Diversity (richness and shannon)

Calculate Hill numbers after equal-coverage standardization.
Based on: https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/2041-210X.12613
and https://onlinelibrary.wiley.com/doi/full/10.1111/oik.07202

```{r}
# use library iNEXT
# Calculate Hill numbers (richness and shannon) by plot:
...
sample_size <- DataInfo(t(sp_plot)) %>%
  select(n)

sample_cov <- DataInfo(t(sp_plot))

richness <- estimateD(t(sp_plot), q= c(0), datatype = "abundance", base = "coverage", nboot = 100)
shannon <- estimateD(t(sp_plot), q= c(1), datatype = "abundance", base = "coverage", nboot = 100)
...
```{r}
# Calculate Hill numbers (richness and shannon) by site
...
richness_site <- estimateD(t(sp_site), q= c(0), datatype = "abundance", base = "coverage", nboot= 100)
shannon_site <- estimateD(t(sp_site), q= c(1), datatype = "abundance", base = "coverage", nboot = 100)
...
richness %>%
  ...
  mutate(plot_id = as.character(Assemblage)) %>%
  right_join(plots_cf, by = "plot_id") %>%
  mutate(hill_richness = as.numeric(qD),
         hill_richness_lcl = as.numeric(qD.LCL),
         hill_richness_ucl = as.numeric(qD.UCL)) %>%
  select(-c(Assemblage, qD, qD.LCL, qD.UCL)) -> plots_cf

shannon %>%
  select(Assemblage, qD, qD.LCL, qD.UCL) %>%
  mutate(plot_id = as.character(Assemblage)) %>%
  right_join(plots_cf, by = "plot_id") %>%
  mutate(hill_shannon = as.numeric(qD),
         hill_shannon_lcl = as.numeric(qD.LCL),
         hill_shannon_ucl = as.numeric(qD.UCL)) %>%
  select(-c(Assemblage, qD, qD.LCL, qD.UCL)) -> plots_cf  
...
richness_site %>%
  select(Assemblage, qD, qD.LCL, qD.UCL) %>%
  mutate(site = as.character(Assemblage)) %>%
  right_join(sites_cf, by = "site") %>%
  mutate(hill_richness = as.numeric(qD),
         hill_richness_lcl = as.numeric(qD.LCL),
         hill_richness_ucl = as.numeric(qD.UCL)) %>%
  select(-c(Assemblage, qD, qD.LCL, qD.UCL)) -> sites_cf
