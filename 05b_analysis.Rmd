...
summary(pca_env)
plot(pca_env)
biplot(pca_env, display = c('species', 'sites'), type = c('text', 'points'), col = c('gray', 'black'))
biplot(pca_env)
...
gamm4_1b <-  gamm4(log1p(agb_plot_ha) ~ t2(hill_shannon, bs="tp", k=4),
                   random = ~(1|site), data = plots_cf_4p)
gamm4_1b_4 <-  gamm4(log1p(agb_plot_ha) ~ s(hill_shannon, bs="tp", k=4),
                   random = ~(1|site), data = plots_cf_4p)

agb_h <- lmer(log1p(agb_plot_ha) ~ hill_shannon + (1|site), data = plots_cf_4p)
stargazer(agb_h,type="text",report=("vc*p"))

agb_h2 <- lmer(log1p(agb_plot_ha) ~ hill_shannon*forest_classification + (1|site), data = plots_cf_4p)
summary(agb_h2)
plot_model(agb_h2)
stargazer(agb_h2,type="text",report=("vc*p"))

agb_h3 <- lmer(log1p(agb_plot_ha) ~ hill_shannon + (1|forest_classification) + (1|site) , data = plots_cf_4p)
summary(agb_h3)
stargazer(agb_h3,type="text",report=("vc*p"))

# Ganador (falta hacer cAICs) --Escribe sobre modelo con y sin forest_classification y sobre la variacion que Site explica. 
gamm4_1b_3 <-  gamm4(log1p(agb_plot_ha) ~ s(hill_shannon, bs= "tp", k= 4),
                   random = ~(1|site) + (1|forest_classification), data = plots_cf_4p)
summary(gamm4_1b_3$gam)

gamm4_1b_5 <-  gamm4(log1p(agb_plot_ha) ~ forest_classification + s(hill_shannon, bs= "tp", k= 4) ,
                   random = ~(1|site), data = plots_cf_4p) #forest class as fixed effect because it has only 4 levels
summary(gamm4_1b_5$gam)
...
gamm4_4_int5 <- gamm4(log1p(agb_plot_ha) ~ env_pc1*agriculture +
           slope*agriculture +
           t2(hill_shannon, bs= "tp", k= 4),
           random = ~(1|site), data = plots_cf_4p)


gamm4_4_int3 <- gamm4(log1p(agb_plot_ha) ~ env_pc1*agriculture +
           slope +
           s(hill_shannon, bs= "tp", k= 4),
           random = ~(1|site), data = plots_cf_4p)

gamm4_4_int4 <- gamm4(log1p(agb_plot_ha) ~ env_pc1*agriculture +
           slope +
...
agbprop <- plots_cf_4p %>%
  ...
  mutate(stem_prop_logit = logit(agb_prop))

lmm_agb_diam <- lmer(stem_prop_logit ~ diam_class + (1|site/plot_id), data = agbprop)
...
#   arrange(diam_class)
# 
# pairwise.wilcox.test(x= agbprop$stem_prop_logit, g= agbprop$diam_class, p.adjust.method = "bonferroni")
# 
...
lmm_agb_diam_fc <- lmer(agb_prop_logit ~ diam_class*forest_classification + (1|site/plot_id), data = agb_prop_fc)
# summary(lmm_agb_diam_str)
#stargazer(lmm_agb_diam_fc, type = "text")
summary(glht(lmm_agb_diam_fc, linfct = mcp(diam_class= "Tukey", forest_classification = "Tukey")), test = adjusted("holm"))
...
agbprop_undis <- agb_prop_fc %>%
  filter(forest_classification == "undisturbed")
agbprop_und_lmm <- lmer(agb_prop_logit ~ diam_class + (1|site), data = agbprop_undis)
summary(agbprop_und_lmm)
stargazer(agbprop_und_lmm, type = "text", report=("vc*p"))
summary(glht(lmer(agb_prop_logit ~ diam_class + (1|site), data = agbprop_undis), linfct = mcp(diam_class = "Tukey"), test = adjusted("holm")))


agbprop_reg <- agb_prop_fc %>%
  filter(forest_classification == "regrowth")
agbprop_reg_lmm <- lmer(agb_prop_logit ~ diam_class + (1|site), data = agbprop_reg)
summary(agbprop_reg_lmm)
stargazer(agbprop_reg_lmm, type = "text", report=("vc*p"))
summary(glht(lmer(agb_prop_logit ~ diam_class + (1|site), data = agbprop_reg), linfct = mcp(diam_class = "Tukey"), test = adjusted("holm")))

agbprop_deg <- agb_prop_fc %>%
  filter(forest_classification == "degraded")
agbprop_deg_lmm <- lmer(agb_prop_logit ~ diam_class + (1|site), data = agbprop_deg)
summary(agbprop_deg_lmm)
stargazer(agbprop_deg_lmm, type = "text", report=("vc*p"))
summary(glht(lmer(agb_prop_logit ~ diam_class + (1|site), data = agbprop_deg), linfct = mcp(diam_class = "Tukey"), test = adjusted("holm")))


agbprop_def <- agb_prop_fc %>%
  filter(forest_classification == "deforested")
agbprop_def_lmm <- lmer(agb_prop_logit ~ diam_class + (1|site), data = agbprop_def)
summary(agbprop_def_lmm)
stargazer(agbprop_def_lmm, type = "text", report=("vc*p"))
summary(glht(lmer(agb_prop_logit ~ diam_class + (1|site), data = agbprop_def), linfct = mcp(diam_class = "Tukey"), test = adjusted("holm")))
```
...
stemprop_undis <- stem_prop_str %>%
  filter(forest_classification == "undisturbed")
stemprop_undis_lmm <- lmer(stem_prop_logit ~ diam_class + (1|site), data = stemprop_undis)
summary(stemprop_undis_lmm)
stargazer(stemprop_undis_lmm, type = "text", report=("vc*p"))
summary(glht(lmer(stem_prop_logit ~ diam_class + (1|site), data = stemprop_undis), linfct = mcp(diam_class = "Tukey"), test = adjusted("holm")))

stemprop_reg <- stem_prop_str %>%
  filter(forest_classification == "regrowth")
stemprop_reg_lmm <- lmer(stem_prop_logit ~ diam_class + (1|site), data = stemprop_reg)
summary(stemprop_reg_lmm)
stargazer(stemprop_reg_lmm, type = "text", report=("vc*p"))
summary(glht(lmer(stem_prop_logit ~ diam_class + (1|site), data = stemprop_reg), linfct = mcp(diam_class = "Tukey"), test = adjusted("holm")))

stemprop_deg <- stem_prop_str %>%
  filter(forest_classification == "degraded")
stemprop_deg_lmm <- lmer(stem_prop_logit ~ diam_class + (1|site), data = stemprop_deg)
summary(stemprop_deg_lmm)
stargazer(stemprop_deg_lmm, type = "text", report=("vc*p"))
summary(glht(lmer(stem_prop_logit ~ diam_class + (1|site), data = stemprop_deg), linfct = mcp(diam_class = "Tukey"), test = adjusted("holm")))

stemprop_def <- stem_prop_str %>%
  filter(forest_classification == "deforested")
stemprop_def_lmm <- lmer(stem_prop_logit ~ diam_class + (1|site), data = stemprop_def)
summary(stemprop_def_lmm)
stargazer(stemprop_def_lmm, type = "text", report=("vc*p"))
summary(glht(lmer(stem_prop_logit ~ diam_class + (1|site), data = stemprop_def), linfct = mcp(diam_class = "Tukey"), test = adjusted("holm")))
```

### Structural attributes, AGB and diversity across disturbance levels
...
stargazer(td_fc,type="text",report=("vc*p"))
plot_model(td_fc, show.p = T, show.values = T)
...
wd_df <- trees_cf_4p %>%
  ...
  right_join(plots_cf_4p) %>%
  filter(!is.na(mean_wd_plot)) %>%
  select(site, plot_id, forest_classification, mean_wd_plot)

wd_fc <- lmer(mean_wd_plot ~ forest_classification + (1|site), data = wd_df)
summary(wd_fc)
stargazer(wd_fc, type = "text")
summary(glht(wd_fc, linfct = mcp(forest_classification = "Tukey")), test = adjusted("holm"))
...
# LMM:
hs_fc <- lmer(hill_shannon ~ forest_classification + (1|site), data = plots_cf_4p)
summary(hs_fc)
stargazer(hs_fc, type = "text")
summary(glht(hs_fc, linfct = mcp(forest_classification = "Tukey")), test = adjusted("holm"))
