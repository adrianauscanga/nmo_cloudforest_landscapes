...
numeric_variables <- plots_cf_4p %>%
  dplyr::select(temp, precip, ai, altitude, latitude, longitude, northness, eastness, slope, t_shannon, sp_richness_plot, agb_plot_ha, agriculture, grazing, logging) %>%
...
```{r}
env <- plots_cf_4p %>%
  dplyr::select(plot_id, temp, precip, ai, altitude, latitude, longitude, northness, eastness, slope)
...
```{r}
# env_pc1, slope, t_shannon, sp_richness_plot, agriculture, grazing, logging

gamm4_1a <- gamm4(log1p(agb_plot_ha) ~ env_pc1, # relationship looks linear so I removed t2
                  random = ~(1|site), data = plots_cf_4p) 

gamm4_1b <-  gamm4(log1p(agb_plot_ha) ~ t2(t_shannon, bs="tp", k=4),
                   random = ~(1|site), data = plots_cf_4p)

gamm4_1c <- gamm4(log1p(agb_plot_ha) ~ t2(sp_richness_plot, bs="tp", k=4),
                   random = ~(1|site), data = plots_cf_4p)

gamm4_1d <- gamm4(log1p(agb_plot_ha) ~ griculture,
                   random = ~(1|site), data = plots_cf_4p)
...
# Best model of 2 predictors is agb ~ sp_richness + agriculture cAIC = 437.158
```
## GAMM: 3 predictors
...
# Model with 6 predictors cAIC= 436.12
```
...
gamm4_4_int <- gamm4(log1p(agb_plot_ha) ~ env_pc1*agriculture +
           slope +
           t2(sp_richness_plot*agriculture, bs= "tp", k=4),
           random = ~(1|site), data = plots_cf_4p)
...
sp_env <- gamm4(sp_richness_plot ~ t2(env_pc1, bs="tp", k=4), random = ~(1|site), data = plots_cf_4p)
summary(sp_env$gam)
plot(sp_env$gam)

sp_ag <- gamm4(sp_richness_plot ~ t2(agriculture, bs="tp", k=4), random = ~(1|site), data = plots_cf_4p)
summary(sp_ag$gam)
plot(sp_ag$gam)
...
plots_cf_4p %>%
  dplyr::select(plot_id, agb_plot, str_class_plots) %>%
  right_join(agb_class_plots, by = "plot_id") %>%
  mutate(agb_prop = agb_class_plot/agb_plot)  %>%
...
agb_prop_str <- plots_cf_4p %>%
  dplyr::select(plot_id, str_class_plots) %>%
...
int <- interaction(agb_prop_str$diam_class, agb_prop_str$succ)
lmm_agb_diam_str <- lmer(agb_prop_logit ~ int + (1|site/plot_id), data = agb_prop_str)
summary(lmm_agb_diam_str)
comp <- glht(lmm_agb_diam_str, linfct = mcp(int = "Tukey"), test = adjusted("holm"))
summary(comp)
...
# summary(lmm_agb_diam_str)
stargazer(lmm_agb_diam_str, type = "text")
...
```{r}
plots_cf_4p %>%
  dplyr::select(plot_id, tree_no, str_class_plots) %>%
  right_join(stems_class_plot, by = "plot_id") %>%
  mutate(stem_prop = stems/tree_no)  %>%
  mutate(succ = ifelse(str_class_plots == "1", "vy",
                       ifelse(str_class_plots == "2", "y", "m"))) %>%
...
int_stem <- interaction(stem_prop_str$diam_class, stem_prop_str$succ)
