...
f4d <- fig4_df %>%
  ggplot(aes(x = mean_height, y = agb_plot_ha))  +
  #geom_smooth(method = "loess", span=1, color = "gray20", se = F, linetype = "dashed", size = 0.75, alpha= 0.3) +
  geom_point(aes(color = factor(succ,
                           levels = c("F","Y", "M"),
                           ordered = TRUE)),
...
f4e <- fig4_df %>%
  ggplot(aes(x = max_height, y = agb_plot_ha))  +
  #geom_smooth(method = "loess", span=1, color = "gray20", se = F, linetype = "dashed", size = 0.75, alpha= 0.3) +
  geom_point(aes(color = factor(succ,
                           levels = c("F","Y", "M"),
                           ordered = TRUE)),
...
summary(lm(shannon ~ landscape, data = sites_cf_4p))
```
...
figure6_2 <- ggarrange(p1,p2,p3,p4,
                  ncol = 2,
                  nrow = 2,
                  labels = c("a)", "b)", "c)", "d)"),
                  font.label = list(size= 10))
