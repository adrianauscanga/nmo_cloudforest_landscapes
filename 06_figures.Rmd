...
# Groupings for VY forests

tukey2p1_vy <- tukey2p1 %>%
  filter(succ == "VY") %>%
...
dif <- tukey2p1_vy$dif
names(dif) <- tukey2p1_vy$comparisons
...
tukey2p2_vy <- tukey2p2 %>%
  filter(succ == "VY") %>%
  select(group1, group2, p.adj.signif) %>%
  unite("comparisons", group1, group2, sep = "-") %>%
  mutate(dif = ifelse(p.adj.signif == "ns", FALSE, TRUE))

difp2_vy <- tukey2p2_vy$dif
names(difp2_vy) <- tukey2p2_vy$comparisons
difp2_vyL <- multcompLetters(difp2_vy)
difp2_vyL
...
figure2 <- ggarrange(p3,p4,p5,p6b,
                  ncol = 2,
                  nrow = 2,
                  labels = c("a)", "b)", "c)", "d)"),
...
## Figure 4. Relationship between AGB and landscape composition, and diversity and landscape composition
...
## Figure 5.  Regressions between AGB and env_pc1 and landscape composition and env_pc1
...
# Figure 6. Loess curves of AGB and tree diversity at site and plot levels
